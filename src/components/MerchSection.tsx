...
export default function MerchSection() {
  return (
    <section className="relative overflow-hidden rounded-[36px] border border-[#E8DEC8] bg-[#F8F1E6] p-6 text-[#111111] shadow-sm sm:p-10 lg:p-12">
      ...
      <div className="relative">
        ...
        <div className="flex flex-col gap-6 border-b border-[#D8CBB7] pb-8 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-black/55 sm:text-base">
              Carry the spirit of Indian dance beyond the studio with
              limited Rhythm of India T-shirts inspired by movement,
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          {products.slice(0, 3).map((product) => (
            ...
            >
              {/* Product image */}
              <Link
                href={`/product-checkout?product=${product.id}&size=M`}
                className="block"
              >
                <div className="relative aspect-[4/4.5] overflow-hidden bg-[#EEE5D7]">
                  <Image
                    src={product.image}
                    alt={product.name}
                    fill
        ...
        <div className="mt-7 flex flex-col gap-3 border-t border-[#D8CBB7] pt-6 text-xs font-bold text-black/45 sm:flex-row sm:items-center sm:justify-between">
          <span>
            PREMIUM CULTURE · EVERYDAY COMFORT · INDIAN ROOTS
          </span>
